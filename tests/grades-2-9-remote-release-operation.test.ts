...
test("remote 0045 activation is exact-tuple, atomic, idempotent and history preserving", () => {
  const sql = operation("ACTIVATE_PUBLIC.sql");
  assert.match(sql, /^\\set ON_ERROR_STOP on[\s\S]*?\nbegin;/u);
  assert.match(sql, /schema_migrations[\s\S]*version = '0045'/u);
...
test("remote release runbook forbids historical activation package and locks server-only flags", () => {
  ...
  assert.match(markdown, /vvseikavrfhjchyrcgqi/u);
  assert.match(markdown, /post-0045\/pre-activation backup/u);
