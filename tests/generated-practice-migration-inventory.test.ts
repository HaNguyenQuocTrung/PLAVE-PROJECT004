...
test("generated persistence inventory pins 0001–0041 without changing the 0001–0040 plan", () => {
  const inventory =
    loadGeneratedPersistenceMigrationInventory();
  assert.equal(inventory.entries.length, 41);
  assert.equal(inventory.entries[0]?.version, "0001");
  assert.equal(inventory.entries.at(-1)?.version, "0041");
  assert.equal(
    inventory.entries.at(-1)?.filename,
    "0041_generated_practice_semantic_provenance.sql",
  );
  assert.equal(
    inventory.entries.at(-1)?.sha256,
...
test("all 41 migration copies are byte-identical", () => {
