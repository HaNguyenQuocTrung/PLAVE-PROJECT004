...
import { parseStartAdaptivePracticeRequest } from "@/lib/practice/adaptive-database-contract";
...
export async function POST(request: Request) {
  ...
  const input = parseStartAdaptivePracticeRequest(rawInput);
  if (!input) {
    return jsonNoStore(adaptiveApiError("INVALID_REQUEST"), 400);
  }
  ...
  const result = await startOrResumeAdaptivePractice(rpc, input);
  return jsonNoStore(
    result,
