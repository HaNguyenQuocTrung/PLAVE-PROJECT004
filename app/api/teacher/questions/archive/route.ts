...
import { archiveTeacherQuestion } from "@/lib/assignments/server";
...
const invalidRequest = {
  ...
  error: {
    ...
    message: "Yêu cầu lưu trữ câu hỏi chưa hợp lệ.",
  },
};
...
export async function POST(request: Request) {
  ...
  const result = await archiveTeacherQuestion(questionId);
  return NextResponse.json(
    result.ok
      ? { ok: true, data: { status: "ARCHIVED" } }
      : {
          ok: false,
