...
export function GeneratorV2OwnerReview({ samples }: { samples: readonly Sample[] }) {
  ...
  async function finalizeReview() {
    ...
    if (!window.confirm(`Ghi quyết định Owner ${overallDecision} cho đủ 198/198 mẫu? Hành động này không tự suy ra từ số phiếu.`)) return;
    ...
    setSaveState("SAVED");
    setSaveMessage("Đã ghi quyết định Owner vào manifest, result JSON và tài liệu trạng thái.");
  }

  return <main className={styles.shell} data-owner-review-root>
    <header className={styles.header}>
      <div><p>PLAVE · Owner review local</p><h1>Đánh giá độ hữu ích Generator V2</h1><span>546 outcomes · 198 canonical capabilities · gói đại diện có giới hạn.</span></div>
      <button onClick={exportReview}>Xuất bản nháp JSON</button>
    </header>

    <section className={styles.progress} aria-label="Tiến độ review">
      ...
      <div><strong>{stats.needsRevision}</strong><span>Needs revision</span></div>
      <div><strong>{stats.remaining}</strong><span>Còn lại</span></div>
    </section>

    <details className={styles.checklist} open>
      <summary>Checklist đánh giá sản phẩm</summary>
      <ul>
        <li>Câu hỏi dễ hiểu và đúng độ tuổi; đúng kỹ năng/outcome.</li>
        <li>Bài toán giải được, đủ dữ kiện; EASY/MEDIUM/HARD khác nhau thực chất.</li>
        <li>Interaction phù hợp; hình, bảng hoặc biểu đồ đúng nghĩa.</li>
        <li>Đáp án sai hợp lý; feedback chỉ ra lỗi và bước tiếp theo.</li>
        <li>Tiếng Việt tự nhiên; không lặp lại, máy móc hoặc vô nghĩa.</li>
      </ul>
    </details>

    <section className={styles.filters} aria-label="Bộ lọc review">
      ...
      <label>Domain<select value={domainFilter} onChange={(event) => setDomainFilter(event.target.value)} data-filter="domain"><option value="ALL">Tất cả</option>{[...new Set(samples.map((item) => item.domain))].sort().map((domain) => <option key={domain} value={domain}>{domain}</option>)}</select></label>
      <label>Mức độ<select value={difficultyFilter} onChange={(event) => setDifficultyFilter(event.target.value)} data-filter="difficulty"><option value="ALL">Tất cả</option><option value="EASY">EASY</option><option value="MEDIUM">MEDIUM</option><option value="HARD">HARD</option></select></label>
      <label>Tương tác<select value={interactionFilter} onChange={(event) => setInteractionFilter(event.target.value)} data-filter="interaction"><option value="ALL">Tất cả</option>{[...new Set(samples.map((item) => item.question.interaction.type))].sort().map((interaction) => <option key={interaction} value={interaction}>{interaction}</option>)}</select></label>
      <label>Trạng thái<select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} data-filter="status"><option value="ALL">Tất cả</option><option value="UNREVIEWED">Chưa đánh giá</option><option value="APPROVE">APPROVE</option><option value="REJECT">REJECT</option><option value="NEEDS_REVISION">NEEDS_REVISION</option></select></label>
    </section>
    ...
    <div className={styles.layout}>
      <aside className={styles.sidebar} aria-label="Danh sách mẫu">
        {filtered.map((item) => <button key={item.sampleId} aria-current={item.sampleId === selected?.sampleId ? "true" : undefined} onClick={() => choose(item.sampleId)} data-sample-id={item.sampleId} data-grade={item.grade} data-domain={item.domain} data-difficulty={item.difficulty} data-interaction={item.question.interaction.type}>
          <strong>Lớp {item.grade} · {item.difficulty} · #{item.sampleNumber}</strong><span>{item.outcomeId}</span><span>{item.title}</span><em>{reviews[item.sampleId]?.decision || "Chưa đánh giá"}</em>
        </button>)}
        {filtered.length === 0 ? <p className={styles.empty}>Không có mẫu khớp bộ lọc.</p> : null}
      </aside>

      {selected ? <section className={styles.review}>
        <div className={styles.navigation}><button disabled={selectedIndex <= 0} onClick={() => move(-1)} data-review-previous>← Previous</button><span>{selectedIndex + 1}/{filtered.length} trong bộ lọc</span><button disabled={selectedIndex >= filtered.length - 1} onClick={() => move(1)} data-review-next>Next →</button></div>
        <div className={styles.meta}><span>{selected.outcomeId}</span><span>{selected.capabilityId}</span><span>Lớp {selected.grade}</span><span>{selected.domain}</span><span>{selected.difficulty}</span><span>{selected.question.interaction.type}</span></div>
        ...
        </div> : <p className={styles.privateNote}>Đáp án và lời giải không có trong browser state trước submit. Feedback chỉ được tạo sau thao tác kiểm tra.</p>}
        <fieldset className={styles.decision} data-sample-decision><legend>Quyết định Owner cho mẫu này</legend>
          {decisions.map((value) => <label key={value}><input type="radio" name={`decision-${selected.sampleId}`} checked={review.decision === value} onChange={() => updateReview({ ...review, decision: value })} />{value}</label>)}
      ...
      </section> : <section className={styles.review}><h2>Không có mẫu khớp bộ lọc</h2><p>Đặt lại một hoặc nhiều bộ lọc để tiếp tục review.</p></section>}
    </div>

    <section className={styles.finalDecision} aria-label="Quyết định Owner tổng thể">
      <div><h2>Quyết định tổng thể</h2><p>Chỉ mở khi reviewed=198. PLAVE không suy ra APPROVE từ đa số phiếu; Owner phải chọn rõ quyết định cuối.</p></div>
      <fieldset disabled={!allReviewed || saveState === "SAVED"}>
        <legend>Owner decision</legend>
        {decisions.map((value) => <label key={value}><input type="radio" name="overall-decision" checked={overallDecision === value} onChange={() => setOverallDecision(value)} />{value}</label>)}
