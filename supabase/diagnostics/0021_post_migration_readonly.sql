-- Read-only verification for the Grade 1 addition-within-20
-- without carrying unit. This statement does not call application
-- mutations or return learner data, answers, or solution content.
with expected_skills(skill_code) as (
  values
    ('ADD_TEN_AND_ONES'::text),
    ('ADD_TEEN_AND_ONES_NO_CARRY'::text),
    ('ADD_USING_TENS_ONES'::text),
    ('ONE_STEP_ADDITION_TO_20'::text)
),
unit_metrics as (
  select
    count(*) filter (
    ...
    ) as addition_to_10_unit_count,
    count(*) filter (
      where unit.slug = 'grade-1-subtraction-within-10'
    ) as subtraction_to_10_unit_count,
    count(*) filter (
      where unit.slug = 'grade-1-numbers-to-20'
    ) as numbers_to_20_unit_count,
    count(*) filter (
      where unit.slug = 'grade-1-addition-within-20-no-carry'
    ) as addition_to_20_unit_count,
    count(*) filter (
      where
    ...
    ) as valid_addition_to_20_unit_count
...
constraint_state as (
  ...
  where
    ...
    and pg_catalog.pg_get_constraintdef(constraint_row.oid)
      like '%ADD_TEN_AND_ONES%'
    and pg_catalog.pg_get_constraintdef(constraint_row.oid)
      like '%ADD_TEEN_AND_ONES_NO_CARRY%'
    and pg_catalog.pg_get_constraintdef(constraint_row.oid)
      like '%ADD_USING_TENS_ONES%'
    and pg_catalog.pg_get_constraintdef(constraint_row.oid)
      like '%ONE_STEP_ADDITION_TO_20%'
),
function_state as (
  select
    count(*) = 1
...
select
  ...
  unit.addition_to_10_unit_count,
  unit.subtraction_to_10_unit_count,
  unit.numbers_to_20_unit_count,
  unit.addition_to_20_unit_count,
  ...
  (
    ...
    and unit.addition_to_10_unit_count = 1
    and unit.subtraction_to_10_unit_count = 1
    and unit.numbers_to_20_unit_count = 1
    and unit.addition_to_20_unit_count = 1
    and unit.valid_addition_to_20_unit_count = 1
    ...
    and constraint_row.skill_constraint_valid
    and routine.prerequisite_guard_valid
...
cross join constraint_state as constraint_row
cross join function_state as routine
