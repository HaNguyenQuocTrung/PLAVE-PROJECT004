-- Read-only verification for the Grade 1 subtraction unit.
-- This statement does not call application RPCs or return learner data.
with expected_skills(skill_code) as (
  values
    ('SUBTRACTION_MEANING'::text),
    ('SUBTRACTION_CALCULATION'::text),
    ('ADDITION_SUBTRACTION_RELATION'::text),
    ('ONE_STEP_SUBTRACTION_WORD_PROBLEM'::text)
),
unit_metrics as (
  select
    count(*) filter (
    ...
    ) as valid_subtraction_unit_count
      ...
      where
        ...
        and (
          ...
          or (
            ...
            and solution.correct_answer !~ '^([0-9]|10)$'
          )
        )
...
constraint_state as (
  ...
  where
    ...
    and pg_catalog.pg_get_constraintdef(constraint_row.oid)
      like '%SUBTRACTION_MEANING%'
    and pg_catalog.pg_get_constraintdef(constraint_row.oid)
      like '%SUBTRACTION_CALCULATION%'
    and pg_catalog.pg_get_constraintdef(constraint_row.oid)
      like '%ADDITION_SUBTRACTION_RELATION%'
    and pg_catalog.pg_get_constraintdef(constraint_row.oid)
      like '%ONE_STEP_SUBTRACTION_WORD_PROBLEM%'
),
function_state as (
  select
    count(*) = 1
...
select
  ...
  (
    ...
    and unit.valid_subtraction_unit_count = 1
