...
const validator = fileURLToPath(new URL("./validate-content-factory-wave-k-shard.ts", import.meta.url));
...
const results = await Promise.all(shards.map((shard) => new Promise<{ shard: string; code: number; output: string }>((resolve) => {
  const child = spawn(process.execPath, ["--no-warnings", "--experimental-strip-types", validator, shard], {
    cwd: process.cwd(), env: { ...process.env, npm_config_offline: "true" }, stdio: ["ignore", "pipe", "pipe"] });
...
const failed = results.filter((result) => result.code !== 0); if (failed.length) throw new Error(`WAVE_K_PARALLEL_SHARDS_FAILED:${failed.map((entry) => entry.shard).join(",")}`);
console.log("WAVE_K_PARALLEL_SHARDS_OK shards=3 grades=9");
