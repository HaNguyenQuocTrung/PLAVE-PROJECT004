...
const allowlist = parseAdaptivePilotEntitlements(
  environment[adaptivePilotEnvironmentKeys.entitlements],
);
...
const allowlistConfigured =
  allowlist.status === "VALID" && allowlist.entitlements.length > 0;

if (mode === "allowlist-count" && allowlistConfigured) {
  console.log("Controlled pilot allowlist: VALID");
  console.log(`Pilot allowlist count: ${allowlist.entitlements.length}`);
