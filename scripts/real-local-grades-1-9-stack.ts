...
function applyMigrations(root: string, home: string, database: string) {
  ...
  if (names.length !== 45 || names[0] !== "0001_auth_profiles.sql" || names.at(-1) !== "0045_grades_2_9_local_public_release.sql") {
...
export async function withRealLocalGradesStack<T>(operation: (stack: RealLocalGradesStack) => Promise<T>) {
  ...
  try {
    ...
    if (query(root, home, database, "select count(*)||'|'||max(version) from supabase_migrations.schema_migrations;") !== "45|0045") fail("REAL_LOCAL_MIGRATION_HISTORY_INVALID");
