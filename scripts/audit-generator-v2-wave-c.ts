...
import {
  ...
  type WaveCNormalizedProblemModel,
...
const inventoryBeforeImplementation = JSON.parse(readFileSync(resolve(output, "outcome-matrix.json"), "utf8")) as { WAVE_C_OUTCOMES?: number; count?: number; canonicalCapabilitiesPlanned?: number; canonicalCapabilities?: number; inventoryRecordedBeforeImplementation?: boolean; rows: readonly Record<string, unknown>[] };
const matrixIds = fullMatrix.rows.filter((row) => row.wave === "C").map((row) => row.outcomeId).sort();
const contractIds = WAVE_C_OUTCOME_CONTRACTS.map((contract) => contract.outcomeId).sort();
...
if (JSON.stringify(matrixIds) !== JSON.stringify(contractIds) || contractIds.length !== 57) throw new Error("WAVE_C_CONTRACT_SET_DOES_NOT_MATCH_LOCKED_TAXONOMY");
if ((inventoryBeforeImplementation.WAVE_C_OUTCOMES ?? inventoryBeforeImplementation.count) !== 57 || (inventoryBeforeImplementation.canonicalCapabilitiesPlanned ?? inventoryBeforeImplementation.canonicalCapabilities) !== 41 || inventoryBeforeImplementation.inventoryRecordedBeforeImplementation === false) throw new Error("WAVE_C_PREIMPLEMENTATION_INVENTORY_MISSING");
if (WAVE_A_OUTCOME_CONTRACTS.length !== 98 || WAVE_B_OUTCOME_CONTRACTS.length !== 61) throw new Error("COMPLETED_WAVE_BASELINE_DRIFT");
...
function incorrectResponse(question: GeneratedProductQuestion): CanonicalResponse {
  ...
  if (typeof correct === "string") return interaction.options?.find((option) => option.id !== correct)?.id ?? `${correct}-sai`;
  if (!Array.isArray(correct) && "numerator" in correct) return { numerator: correct.numerator + correct.denominator, denominator: correct.denominator };
  if (Array.isArray(correct) && correct.every((item) => typeof item === "string")) return correct.length > 1 ? [...correct].reverse() : ["incorrect"];
  if (Array.isArray(correct)) return correct.map((pair, index) => ({ leftId: pair.leftId, rightId: correct[(index + 1) % correct.length]!.rightId }));
  return "incorrect";
}
...
for (const contract of WAVE_C_OUTCOME_CONTRACTS) {
  ...
  if (!official || official.grade !== contract.grade || official.mappedUnitIds.length === 0) throw new Error(`WAVE_C_OFFICIAL_MAPPING_INVALID:${contract.outcomeId}`);
  ...
  registryRows.push({ capabilityId: contract.canonicalVariantId, outcomeId: contract.outcomeId, grade: contract.grade, contractVersion: contract.contractVersion, taskMode: contract.taskMode, interactions: contract.interactionPolicy, deterministicSeedContract: "sha256(outcomeId:difficulty:seed)", promptModel: contract.normalizedProblemModel, answerModel: contract.acceptedAnswerPolicy, visualModel: contract.visualPolicy, solver: contract.independentSolver, validator: contract.independentValidator, distractorValidator: "WAVE_C_DISTRACTOR_UNIQUENESS_AND_FALSEHOOD_V2", feedbackStrategy: contract.feedbackPolicy, misconceptions: contract.misconceptionCatalog, provenance: "GENERATED_V2_8_OF_8" });
  ...
  for (const difficulty of difficulties) {
    ...
    for (let index = 1; index <= 20; index += 1) {
      const seed = `s8cc-${contract.outcomeId.toLowerCase()}-${difficulty.toLowerCase()}-${String(index).padStart(2, "0")}`;
    ...
    if (exactDuplicateRate !== 0 || nearDuplicatePairRate > 0.12 || structures.size < 1) throw new Error(`WAVE_C_DIVERSITY_FAILED:${contract.outcomeId}:${difficulty}:${exactDuplicateRate}:${nearDuplicatePairRate}`);
    ...
    if (!correctFeedback.isCorrect || incorrectFeedback.isCorrect) throw new Error(`WAVE_C_FEEDBACK_FAILED:${contract.outcomeId}:${difficulty}`);
    const persisted = to0041Question(representative!, { position: 1, releaseId: "plave-math-grades-1-9-v1", unitId: official.mappedUnitIds[0]!, skillId: `WAVE_C_${contract.canonicalVariantId}`, skillTitle: contract.measurableIntent, contentReleaseHash: "c".repeat(64) });
    if (persisted.question.visual.productContract.questionSource !== "GENERATED_V2" || JSON.stringify(persisted.question).includes("correctResponse")) throw new Error(`WAVE_C_PERSISTENCE_BOUNDARY_FAILED:${contract.outcomeId}:${difficulty}`);
...
if (sampleCount !== 3_420 || provenanceCount !== sampleCount || persistenceProofCount !== 171 || capabilities.size !== 41) throw new Error("WAVE_C_AUDIT_TOTAL_INVALID");

const negativeControls: unknown[] = [];
for (const contract of WAVE_C_OUTCOME_CONTRACTS.filter(isWaveCImplementedByNewEngine)) {
  const inspected = __waveCNegativeControl.inspect(contract, { outcomeId: contract.outcomeId, grade: contract.grade, difficulty: "HARD", seed: `negative-${contract.outcomeId.toLowerCase()}`, locale: "vi-VN" });
  const controls: string[] = [];
  const reject = (name: string, action: () => unknown) => { try { action(); } catch { controls.push(name); return; } throw new Error(`WAVE_C_NEGATIVE_NOT_REJECTED:${contract.outcomeId}:${name}`); };
  reject("PROMPT_MODEL_MISMATCH", () => __waveCNegativeControl.validate(contract, inspected.normalizedModel, inspected.solution, "Prompt không thuộc mô hình", inspected.interaction, inspected.visual));
  const wrong = { ...inspected.solution, correct: typeof inspected.solution.correct === "number" ? inspected.solution.correct + 1 : "incorrect" as CanonicalResponse, accepted: [typeof inspected.solution.correct === "number" ? inspected.solution.correct + 1 : "incorrect" as CanonicalResponse] };
  reject("INDEPENDENT_SOLVER_MISMATCH", () => __waveCNegativeControl.validate(contract, inspected.normalizedModel, wrong, inspected.prompt, inspected.interaction, inspected.visual));
  reject("OUT_OF_GRADE_PARAMETER", () => __waveCNegativeControl.validate(contract, { ...inspected.normalizedModel, values: [1_000_000_000, ...inspected.normalizedModel.values.slice(1)] }, inspected.solution, inspected.prompt, inspected.interaction, inspected.visual));
  if (inspected.normalizedModel.rationals.length) {
    const tampered = structuredClone(inspected.normalizedModel) as WaveCNormalizedProblemModel;
    (tampered as { rationals: FractionValue[] }).rationals[0] = { numerator: 1, denominator: 0 };
    reject("ZERO_DENOMINATOR", () => __waveCNegativeControl.validate(contract, tampered, inspected.solution, inspected.prompt, inspected.interaction, inspected.visual));
  }
...
const report = {
  schemaVersion: 1, generatedAt: "2026-08-02", sprint: "8C.C", result: browserPassed ? "PASS_BROWSER_VALIDATED" : "BLOCKED_BROWSER_ACCEPTANCE_PENDING",
  roadmap: { milestone1: "COMPLETE_OWNER_APPROVED", milestone2: "IN_PROGRESS_RESUMED", milestone3: "COMPLETE_OWNER_APPROVED_LOCAL_MVP" },
  inventory: { recordedBeforeImplementation: true, taxonomySource: "artifacts/generator-v2-full-coverage/outcome-matrix.json", taxonomyBoundary: "EXPLICIT_WAVE_C_FIELD_NO_REPARTITION", officialDenominator: 546, exactWaveCOutcomes: 57, grades: [...new Set(WAVE_C_OUTCOME_CONTRACTS.map((contract) => contract.grade))].sort(), canonicalCapabilities: capabilities.size, exactOutcomeIds: contractIds, metadataInsufficientOutcomes: [] },
  coverage: { waveCTotal: 57, implementedReviewRequired: 57, blocked: 0, mathematicallyUnsolvableOutcomes: [], generatedSamples: sampleCount, expectedSamples: 3_420, fallbackCount: 0, keywordRoutingCount: 0, overallGeneratorV2ImplementedOutcomes: GENERATOR_V2_OUTCOME_REGISTRY.length, fullCanonicalOutcomeTotal: 546, remainingPostWaveCOutcomes: 546 - GENERATOR_V2_OUTCOME_REGISTRY.length, claimsMilestone2Complete: false },
  contracts: { type: "PLAVE_PRODUCT_ASSESSMENT_CONTRACT_V2", version: "wave-c-v2.1", explicitOutcomeRouting: true, canonicalCapabilities: capabilities.size, newlyImplementedOutcomes: 56, preservedProvenBaseline: 1, genericFallback: false, syntheticAliases: false, runtimeLlmAnswering: false },
  validation: { independentlySolvedAndValidated: sampleCount, provenance8Of8: provenanceCount, publicBoundary: "PASS", deterministicReplay: "PASS", exactDuplicates: 0, maximumNearDuplicatePairRate, difficultyStructuralGates: "PASS", multidimensionalDiversity: "PASS", persistenceAdapterProofs: persistenceProofCount, exactArithmetic: "INTEGER_RATIONAL_COEFFICIENT_MODELS", symbolicValidation: "CANONICAL_COEFFICIENT_AND_ROOT_RECOMPUTATION", decimalArithmetic: "INTEGER_SCALED", fractionEquivalence: "GCD_NORMALIZED" },
  negativeControls: { status: "PASS", outcomesTested: negativeControls.length, capabilityFamilies: capabilities.size, evidence: "artifacts/generator-v2-wave-c/negative-controls.json" },
  ...
  boundaries: ["NO_REMOTE_ACCESS_OR_MUTATION", "NO_MIGRATION_CHANGE", "NO_DEPLOYMENT", "NO_GIT_MUTATION", "NO_AI_TUTOR_IMPLEMENTATION_CHANGE", "NO_DEFAULT_RUNTIME_ENABLE", "NO_GENERIC_FALLBACK", "NO_WAVE_D_IMPLEMENTATION"],
};

writeFileSync(resolve(output, "outcome-matrix.json"), `${JSON.stringify({ schemaVersion: 1, sprint: "8C.C", taxonomyBoundary: "EXPLICIT_WAVE_C_FIELD_NO_REPARTITION", inventoryRecordedBeforeImplementation: true, count: outcomeRows.length, canonicalCapabilities: capabilities.size, rows: outcomeRows }, null, 2)}\n`);
...
writeFileSync(resolve(output, "negative-controls.json"), `${JSON.stringify({ schemaVersion: 1, result: "PASS", outcomeControls: negativeControls, unsupportedOutcome: "GENERATOR_V2_OUTCOME_NOT_IMPLEMENTED", capabilityFamiliesCovered: capabilities.size }, null, 2)}\n`);
writeFileSync(resolve(output, "review-manifest.json"), `${JSON.stringify(reviewManifest, null, 2)}\n`);
writeFileSync(resolve(output, "report.json"), `${JSON.stringify(report, null, 2)}\n`);

console.log("WAVE_C_OUTCOMES=57/57"); console.log(`WAVE_C_CAPABILITIES=${capabilities.size}`); console.log(`WAVE_C_SAMPLES=${sampleCount}/3420`); console.log(`WAVE_C_MAX_NEAR_DUPLICATE=${maximumNearDuplicatePairRate.toFixed(6)}`); console.log(`WAVE_C_BROWSER=${browserPassed ? "PASS" : "PENDING"}`);
